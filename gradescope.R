...
strawb_organic
#Q2
a3 <- strawb_organic[strawb_organic$State=="CALIFORNIA",]
a4 <- a3[a3$Year==2016,]
#View(a4)
a4$`CV (%)` <- as.numeric(a4$`CV (%)`)
n <- 6
estimate <- mean(a4$`CV (%)`)
#estimate
sd <- sd(a4$`CV (%)`) 
...
estimate + 1.96 * sd / 6
#Q3
a5 <- strawb_non_organic[strawb_non_organic$State=="CALIFORNIA",]
a6 <- a5[a5$Year==2016,]
#View(a4)
a6$`CV (%)` <- as.numeric(a6$`CV (%)`)
estimate <- mean(a6$`CV (%)`)
estimate
n <- 13
sd <- sd(a6$`CV (%)`) 
...
#Q4
a1 <- strawb_chem[strawb_chem$State=="CALIFORNIA",]
length(unique(a1$chem_name))
#Q5
a2 <- strawb_chem[strawb_chem$State=="FLORIDA",]
length(unique(a2$chem_name))
length(unique(a1$chem_name))-length(unique(a2$chem_name))
